...
describe('Layout', () => {
  ...
  it('does not dipslay the label when no label provided in props', () => {
    const { container } = render(<Input />);
  ...
  it('has value for input when it is provided as prop', () => {
    const { container } = render(<Input type="password" value="Test value" />);
    const input = container.querySelector('input');
    expect(input.value).toBe('Test value');
  });

  it('has onChange callback when it is provided as prop', () => {
    // use mock function because there is noChange property so we
    // simulate it using a mock
  ...
  it('default style when there is no validation error or success', () => {
    const { container } = render(<Input />);
    const input = container.querySelector('input');
